...
export const SudokuBoard: React.FC<SudokuBoardProps> = ({
  ...
  return (
    <div className="rounded-[2rem] border border-white/80 bg-[rgba(255,255,255,0.78)] p-4 shadow-[0_24px_60px_rgba(15,23,42,0.12)] backdrop-blur-sm sm:p-5">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Grid</div>
          <div className="mt-1 font-display text-2xl uppercase tracking-[0.08em] text-slate-900">Board Matrix</div>
        </div>
      ...
      <div className="relative mx-auto aspect-square w-full max-w-[42rem] overflow-hidden rounded-[1.6rem] border border-[#b9caef] bg-[#d7e5ff] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.75)] sm:p-4">
        <div className="grid h-full w-full grid-cols-9 grid-rows-9 overflow-hidden rounded-[1rem] border-2 border-slate-900 bg-slate-900">
          {board.map((row, r) =>
            row.map((cell, c) => {
        ...
        {status === 'completed' && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-emerald-100/75 backdrop-blur-sm">
            <div className="rounded-[1.6rem] border border-emerald-200 bg-white/90 px-8 py-7 text-center shadow-[0_24px_60px_rgba(15,118,110,0.15)]">
              ...
              <span className="block font-mono text-xl text-emerald-950">{formatScore(gameState.timer)}</span>
              <span className="mt-3 block text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700">Final Score</span>
              <span className="block font-display text-3xl uppercase tracking-[0.06em] text-emerald-900">{gameState.timer}</span>
